package com.hk.heichijun.utils.print;
...
public class AidlUtil {
    ...
    /**
     * 打印文字 可设置字号 是否加粗 对齐方式
     */
    public void printMyText(String content, float size, boolean isBold, int alignType, int lineNum) {
        if (woyouService == null) {
            Toast.makeText(context,R.string.toast_2,Toast.LENGTH_LONG).show();
            return;
        }
        try {
            ...
            //设置对齐模式  0--居左 , 1--居中, 2--居右
            woyouService.setAlignment(alignType,null);
            woyouService.printTextWithFont(content, null, size, null);
            woyouService.lineWrap(lineNum, null);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
