...
import com.hk.heichijun.utils.SingletonRequestQueue;
...
public class SortAdapter2 extends BaseAdapter implements SectionIndexer {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null) {
			...
			view = LayoutInflater.from(mContext).inflate(R.layout.phone_constacts_item2, null);
		...
		view.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if(getDataListener!=null) {
					getDataListener.onGetData(list.get(position).getId());
				}
			}
		});
		viewHolder.tvTitle.setText(this.list.get(position).getName());
		SingletonRequestQueue.getInstance().loadImageByVolley(viewHolder.icon,this.list.get(position).getUrl());
		return view;
	}

	public interface GetDataListener{
		void onGetData(String id);
